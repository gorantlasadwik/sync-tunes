...
if GEMINI_API_KEY:
    ...
    print(f"SUCCESS: Gemini API configured with key: {GEMINI_API_KEY[:10]}...")

# Configure Groq API
GROQ_API_KEY = os.getenv('GROQ_API_KEY')
if GROQ_API_KEY:
    print(f"SUCCESS: Groq API configured with key: {GROQ_API_KEY[:10]}...")
else:
    print("WARNING: Groq API key not found - will use fallback parsing only")

# Initialize YouTube Music API
try:
    ytmusic = YTMusic()
    print("SUCCESS: YouTube Music API initialized successfully")
except Exception as e:
    print(f"WARNING: YouTube Music API initialization failed: {e}")
    ytmusic = None

def generate_captcha():
    """Generate a random CAPTCHA string with mixed case, numbers, and symbols"""
    ...
    # Generate random length between 5-7 characters
    length = random.randint(5, 7)
    
    # Generate random string
    captcha_string = ''.join(random.choice(all_chars) for _ in range(length))
    
    return captcha_string

def verify_captcha(user_input, session_captcha):
    """Verify CAPTCHA input (case-sensitive)"""
    if not user_input or not session_captcha:
        return False
    return user_input.strip() == session_captcha.strip()
...
def send_otp_email(email, otp, user_name):
    ...
    try:
        msg = Message(
            subject='Sync Tunes - Password Reset OTP',
            recipients=[email],
            sender=(os.getenv('MAIL_SENDER_NAME', 'Sync Tunes'), os.getenv('MAIL_SENDER_EMAIL', 'sync.tunes.auth@gmail.com'))
        )
        ...
        <head>
            <style>
                ...
                .otp-box {{ background-color: #f8f9fa; border: 2px dashed #1db954; border-radius: 8px; padding: 20px; text-align: center; margin: 20px 0; }}
                .otp-code {{ font-size: 32px; font-weight: bold; color: #1db954; letter-spacing: 5px; font-family: 'Courier New', monospace; }}
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>You requested a password reset for your Sync Tunes account. Use the OTP below to verify your identity:</p>
                    ...
                    <p>Best regards,<br>The Sync Tunes Team</p>
                </div>
                <div class="footer">
                    <p>© 2024 Sync Tunes. All rights reserved.</p>
...
def check_and_reset_gemini_quota():
    ...
    if quota_exceeded and quota_reset_time:
        ...
        if time_since_reset.total_seconds() >= 24 * 60 * 60:  # 24 hours
            ...
            print(f"RESET: Gemini quota automatically reset after 24 hours for user {current_user.user_id}")
    
    # If quota is exceeded but we haven't set a reset time, set it now
    elif quota_exceeded and not quota_reset_time:
        session['gemini_quota_reset_time'] = current_time.isoformat()
        print(f"TIMER: Gemini quota exceeded - will auto-reset in 24 hours for user {current_user.user_id}")
...
def get_licensed_metadata(video_metadata):
    ...
    if "licensed" in str(video_metadata).lower():
        ...
        print("MUSIC: Found licensed metadata")
...
def get_from_ytmusic(query):
    ...
    if not ytmusic:
        print("WARNING: YouTube Music API not available")
        return None
    
    try:
        print(f"MUSIC: Searching YouTube Music for: '{query}'")
        ...
        if results and len(results) > 0:
            ...
            if song_name and artist_name:
                print(f"SUCCESS: YouTube Music found: '{song_name}' by '{artist_name}'")
                return {
                    'title': song_name,
                    'artist': artist_name,
...
# SMTP settings via environment variables (use your verified sender)
...
app.config['MAIL_DEFAULT_SENDER'] = (os.getenv('MAIL_SENDER_NAME', 'Sync Tunes'), os.getenv('MAIL_SENDER_EMAIL', 'sync.tunes.auth@gmail.com'))
...
def login():
    ...
    if request.method == 'POST':
        ...
        # Verify CAPTCHA
        session_captcha = session.get('captcha_word')
        if not verify_captcha(captcha_input, session_captcha):
            captcha_error = 'Invalid security check. Please try again.'
            # Generate new CAPTCHA for retry
            new_captcha = generate_captcha()
            session['captcha_word'] = new_captcha
            return render_template('login.html', 
                                 captcha_word=new_captcha,
    ...
    session['captcha_word'] = captcha_word
...
def verify_otp():
    ...
    if 'otp' not in session or 'otp_email' not in session:
        flash('Invalid or expired OTP session. Please request a new OTP.')
        return redirect(url_for('forgot_password'))
    
    # Check OTP expiry (10 minutes)
    otp_timestamp = datetime.fromisoformat(session['otp_timestamp'])
    if datetime.now() - otp_timestamp > timedelta(minutes=10):
    ...
    if request.method == 'POST':
        ...
        if user_otp == stored_otp:
            ...
            return render_template('verify_otp.html', 
                                 email=session.get('otp_email', '').split('@')[0] + '@***')
